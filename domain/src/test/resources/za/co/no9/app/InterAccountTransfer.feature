Feature: Inter-account Funds Transfer
  ...
  Provide the ability to allow clients to move money between different accounts. This feature does not consider
  the more general scenario of transferring money between accounts that do not belong to the client.

  Background:
    Given a registered client with the user name andrew
    And andrew has a current account 10001235 with opening balance R123.45
    And andrew has a current account 10001236 with opening balance R0.00
    And andrew has a current account 10001237 with opening balance USD0.00
  ...
  Scenario: Successful transfer
    ...
    And andrew has an inter-account transfer audit entry:
      | name                | value        |
      | amount              | ZAR100.00    |
